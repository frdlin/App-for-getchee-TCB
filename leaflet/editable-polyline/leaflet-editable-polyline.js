    ...
    _addMethods: function () {
        ...
        */
        ...
        this._parseOptions = function (options) {
            ...
            if (!options.pointIcon)
                this._options.pointIcon = L.icon({ iconUrl: 'editmarker.png', iconSize: [49, 49], iconAnchor: [25, 25] });
            if (!options.newPointIcon)
                this._options.newPointIcon = L.icon({ iconUrl: 'editmarker2.png', iconSize: [49, 49], iconAnchor: [25, 25] });
        };
        ...
        */
        this._reloadPolyline = function (fixAroundPointNo) {
            var newLatLngs = that._getMarkerLatLngs();
            if (fixAroundPointNo == this._markers.length - 1) {
                newLatLngs[0].lat = newLatLngs[this._markers.length - 1].lat;
                newLatLngs[0].lng = newLatLngs[this._markers.length - 1].lng;
            }
            else if (fixAroundPointNo == 0) {
                newLatLngs[this._markers.length - 1].lat = newLatLngs[0].lat;
                newLatLngs[this._markers.length - 1].lng = newLatLngs[0].lng;
            }

            //that.setLatLngs(that._getMarkerLatLngs());
            that.setLatLngs(newLatLngs);
        ...
        */
        this._addMarkers = function (pointNo, latLng, fixNeighbourPositions) {
            ...
            marker.on('dragstart', function (event) {
                ...
                var previousPoint = pointNo && pointNo > 0 ? that._markers[pointNo - 1].getLatLng() : (pointNo == 0 && that._markers.length > 2 ? that._markers[that._markers.length - 2].getLatLng() : null);
                var nextPoint = pointNo < that._markers.length - 1 ? that._markers[pointNo + 1].getLatLng() : (that._markers.length > 2 ? that._markers[1].getLatLng() : null);
...
//            });
            ...
            newPointMarker.on('dragend', newMarkerDragEnd);
...
//            });
        ...
        var newMarkerDragEnd = function (event) {
            var marker = event.target;
            var pointNo = that._getPointNo(event.target);
            that._addMarkers(pointNo, marker.getLatLng(), true);
            setTimeout(function () {
        ...
        */
        this._fixAround = function (pointNoOrMarker) {
            ...
            //alert(pointNo + ',' + that._markers.length);
            ...
            var previousMarker = pointNo == 0 ? (that._markers.length > 2 ? that._markers[that._markers.length - 2] : null) : that._markers[pointNo - 1];
            var nextMarker = pointNo < that._markers.length - 1 ? that._markers[pointNo + 1] : (that._markers.length > 2 ? that._markers[1] : null);
